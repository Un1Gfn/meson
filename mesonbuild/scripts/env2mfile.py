...
def write_machine_file(infos: MachineInfo, ofilename: str) -> None:
...
def detect_cross_envvars(options: Any) -> MachineInfo:
    infos = MachineInfo()
    cppflags = tuple(shlex.split(os.environ.get('CPPFLAGS', '')))
    ldflags = tuple(shlex.split(os.environ.get('LDFLAGS', '')))
    for lang, envvarname in UNIXY_ENVVARS_COMPILER.items():
        compilerstr = os.environ.get(envvarname)
        if not compilerstr:
            continue
        compiler = shlex.split(compilerstr)
        infos.compilers[lang] = compiler
        compile_args = shlex.split(os.environ.get(UNIXY_ENVVARS_FLAGS[lang], ''))
        if lang in LANGS_USING_CPPFLAGS:
            lang_args = list(cppflags) + compile_args
        else:
            lang_args = compile_args
        lang_link_args = list(ldflags) + compile_args
        if lang_args:
            infos.compile_args[lang] = lang_args
        if lang_link_args:
            infos.link_args[lang] = lang_link_args
...
def detect_cross_env(options: Any) -> MachineInfo:
    if options.debarch:
        ...
        infos = detect_cross_envvars(options)
    return infos
...
def run(options: Any) -> None:
    ...
    write_machine_file(infos, options.outfile)
